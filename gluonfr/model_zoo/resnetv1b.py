...
class BasicBlockV1b(HybridBlock):
    ...
    def __init__(self, planes, strides=1, dilation=1, downsample=None,
                 previous_dilation=1, norm_layer=None, activation_type='relu', norm_kwargs=None, **kwargs):
        ...
        if activation_type == 'prelu':
            ...
            self.relu1 = nn.Activation(activation_type)
        ...
        if activation_type == 'prelu':
            ...
            self.relu2 = nn.Activation(activation_type)
...
class BottleneckV1b(HybridBlock):
    ...
    def __init__(self, planes, strides=1, dilation=1,
                 downsample=None, previous_dilation=1, norm_layer=None, activation_type='relu',
        ...
        if activation_type == 'prelu':
            ...
            self.relu3 = nn.Activation(activation_type)
        self.downsample = downsample
...
class FaceResNetV1b(HybridBlock):
    """ Pre-trained ResNetV1b Model, which produces the strides of 8
    featuremaps at conv5.
    ...
    def __init__(self, block, layers, classes=1000, embedding_size=512, dilated=False,
                 norm_layer=BatchNorm, activation_type='relu', norm_kwargs=None, last_gamma=False, deep_stem=False,
                 stem_width=32, avg_down=False, final_drop=0.4, use_global_stats=False,
                 name_prefix='', need_cls_layer=True, weight_norm=False, feature_norm=False, **kwargs):
        self.inplanes = stem_width*2 if deep_stem else 64
        super(FaceResNetV1b, self).__init__(prefix=name_prefix)
        norm_kwargs = norm_kwargs if norm_kwargs is not None else {}
        if use_global_stats:
            norm_kwargs['use_global_stats'] = True
        self.norm_kwargs = norm_kwargs
        ...
        with self.name_scope():
            ...
            if activation_type == 'prelu':
                ...
                self.relu = nn.Activation(activation_type)
            self.maxpool = nn.MaxPool2D(pool_size=3, strides=2, padding=1)
            # self.input = nn.HybridSequential(prefix='input_')
            # with self.input.name_scope():
            #     self.input.add(self.conv1,
            #                    self.bn1,
            #                    self.relu,
            #                    self.maxpool)
            self.stage1 = self._make_layer(1, block, 64, layers[0], avg_down=avg_down,
                                           norm_layer=norm_layer, activation_type=activation_type, last_gamma=last_gamma)
            self.stage2 = self._make_layer(2, block, 128, layers[1], strides=2, avg_down=avg_down,
                                           norm_layer=norm_layer, activation_type=activation_type, last_gamma=last_gamma)
            if dilated:
                self.stage3 = self._make_layer(3, block, 256, layers[2], strides=1, dilation=2,
                                               avg_down=avg_down, norm_layer=norm_layer, activation_type=activation_type,
                                               last_gamma=last_gamma)
                self.stage4 = self._make_layer(4, block, 512, layers[3], strides=1, dilation=4,
                                               avg_down=avg_down, norm_layer=norm_layer, activation_type=activation_type,
                                               last_gamma=last_gamma)
            else:
                self.stage3 = self._make_layer(3, block, 256, layers[2], strides=2,
                                               avg_down=avg_down, norm_layer=norm_layer, activation_type=activation_type,
                                               last_gamma=last_gamma)
                self.stage4 = self._make_layer(4, block, 512, layers[3], strides=2,
                                               avg_down=avg_down, norm_layer=norm_layer, activation_type=activation_type,
                                               last_gamma=last_gamma)

            # embedding pattern differs from original ResNet for **face recognition**
            # use BN-DropOut-FC-BN instead of Global Pooling
            self.embedding = nn.HybridSequential(prefix='embedding_')
            with self.embedding.name_scope():
                self.embedding.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
                self.embedding.add(nn.Flatten())
                if final_drop > 0.0:
                    self.embedding.add(nn.Dropout(final_drop))
                self.embedding.add(nn.Dense(embedding_size, use_bias=False))
                self.embedding.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
            if need_cls_layer:
    ...
    def _make_layer(self, stage_index, block, planes, blocks, strides=1, dilation=1,
                    avg_down=False, norm_layer=None, activation_type='relu', last_gamma=False):
        downsample = None
        if strides != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.HybridSequential(prefix='down%d_'%stage_index)
            with downsample.name_scope():
        ...
        with layers.name_scope():
            if dilation in (1, 2):
                layers.add(block(planes, strides, dilation=1,
                                 downsample=downsample, previous_dilation=dilation,
                                 norm_layer=norm_layer, norm_kwargs=self.norm_kwargs,
                                 last_gamma=last_gamma, activation_type=activation_type))
            elif dilation == 4:
                layers.add(block(planes, strides, dilation=2,
                                 downsample=downsample, previous_dilation=dilation,
                                 norm_layer=norm_layer, norm_kwargs=self.norm_kwargs,
...
def resnet101_v1c(pretrained=False, root='~/.mxnet/models', ctx=cpu(0), **kwargs):
    ...
    norm_layer : object
        ...
        Can be :class:`mxnet.gluon.nn.BatchNorm` or :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 23, 3], deep_stem=True, activation_type='prelu',
                      name_prefix='resnetv1c_', **kwargs)
    if pretrained:
        from .model_store import get_model_file
...
def face_resnet50_v1d(pretrained=False, root='~/.mxnet/models', ctx=cpu(0), **kwargs):
    """Constructs a ResNetV1d-50 model.
